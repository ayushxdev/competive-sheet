...
public class chewbacca {
  public static void main(String[] args) {
    ...
    StringBuilder req=new StringBuilder();// string is not mutable it's mutable in which ans will be strored
    for (int i = 0; i < number.length(); i++) {// string is fixed but stringbulder is not 
      int n=number.charAt(i)-'0';//number-string=number ==> conterving string to number
      int n_2=(9-n);
      if (i==0 && n==9) {//==> 1st digit
        // is 1st digit (9) become 0 make no sense
        req.append(n);// append fxn ==> in string bulder it's used to add new string
        }
        else{
          req.append(Math.min(n, n_2));//b/w  t and (9-t) which is is smaller append that
        }// append meas adding it 
    ...
    System.out.println(req.toString());//we giving it as string cz go trough of all number is not possible that's why we have to use it 
    sc.close();
  }
